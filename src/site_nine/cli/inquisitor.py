...
from site_nine.inquisitor.manager import InquisitorManager, get_fail_hints, get_fail_summary
from site_nine.inquisitor.models import Severity
from site_nine.inquisitor.rendering import (
    infra_subject_color,
...
def inquisitor_command(
    fix: Annotated[bool, typer.Option("--fix", help="Apply fixes automatically")] = False,
    verbose: Annotated[bool, typer.Option("--verbose", "-v", help="Show detailed output")] = False,
    older_than: Annotated[
        int, typer.Option("--older-than", help="Hours threshold for rogue possession detection (default: 3)")
    ] = 3,
) -> None:
    """Run health checks and validate data integrity (typically used by: humans, Administrators)
    ...
    - Incorrect daemon incarnation counters
    - Missing files
    - Abandoned work detection
    - Rogue possession detection (auto-exorcise with --fix)

    By default, only reports issues. Use --fix to automatically repair safe issues,
    including auto-exorcising rogue possessions and releasing their tasks.
    """
    try:
    ...
    terminal_message("Running diagnostics...", subject="Inquisitor")

    with Database(db_path) as db:
        manager = InquisitorManager(db, opencode_dir)
        report = manager.run_diagnostics(verbose=verbose, stale_hours=older_than)
