...
class MinionWorker:
    ...
    def __init__(
        ...
        Args:
            ...
            spawn_token: Optional UUID token passed by summon_minion.py.  When
                provided, the worker writes a JSON status file at
                ``~/.local/state/site-nine/workers/<spawn_token>.json`` after
                initialization so that summon_minion.py can retrieve the exact
                possession ID without racing against other concurrent spawns.
        """
        self.role = role
        self.daemon = daemon
        self.model = model or self.DEFAULT_MODEL
        self.poll_interval = poll_interval
        self.spawn_token: Optional[str] = spawn_token
    ...
    def _write_spawn_token_file(self) -> None:
        """Write spawn token status file so summon_minion.py can find the exact possession.

        When the worker was started with a ``--spawn-token`` argument, this method
        writes a small JSON file to ``~/.local/state/site-nine/workers/<token>.json``
        containing the session ID, possession ID, daemon name, and status ``"ready"``.

        ``summon_minion.py`` polls for this file instead of querying the database by
        role, which eliminates the race condition that occurs when two same-role workers
        are spawned concurrently.

        If no spawn token was provided (e.g., direct CLI invocation), this is a no-op.
        """
        if not self.spawn_token:
            return

        status_dir = Path.home() / ".local" / "state" / "site-nine" / "workers"
        status_dir.mkdir(parents=True, exist_ok=True)
        status_file = status_dir / f"{self.spawn_token}.json"
        ...
        # Write atomically: write to a temp file then rename so summon_minion.py
...
Examples:
  ...
  minion-worker.py Operator --model github-copilot/claude-opus-4
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "UUID token generated by summon_minion.py.  When provided, the worker "
            "writes a JSON status file to "
            "~/.local/state/site-nine/workers/<spawn-token>.json after initialization "
            "so summon_minion.py can retrieve the possession ID without a race condition."
        ),
    )
