# Task OPR-H-0016: Make s9 mission end command the source of truth for session status
...
Currently agents can manually edit session file YAML frontmatter (status, end_time) without running 's9 mission end', causing database and file to get out of sync. This leads to orphaned sessions that appear active in the database but are marked completed in files.

Solution: Make 's9 mission end' the single source of truth by:
...
3. Document that only 's9 mission end' should update session status
