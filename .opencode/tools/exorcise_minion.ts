...
export default tool({
  ...
  args: {
    from_possession_id: tool.schema.number().describe("The possession ID sending the exorcism signal (typically Admin/Director)"),
    to_possession_id: tool.schema.number().describe("The possession ID of the worker to exorcise"),
    reason: tool.schema
      .string()
      .optional()
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".opencode/tools/exorcise_minion.py")
