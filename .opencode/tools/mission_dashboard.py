#!/usr/bin/env python3
"""
mission_dashboard tool - Show role-filtered task dashboard.
...
def main():
    try:
        ...
        logger.debug("mission_dashboard_called", role=role_raw)

        # Validate role
        try:
            role_enum = Role.from_string(role_raw)
        except ValueError:
            logger.warning("mission_dashboard_invalid_role", role=role_raw)
            return json.dumps(
                {
                    "error": "invalid_role",
        ...
        logger.info("mission_dashboard_returned", role=role_enum.title_case, task_count=len(available))
        ...
        logger.exception("mission_dashboard_error", error=str(e))
