...
def test_inquisitor_with_verbose_and_fix(project_dir: Path, test_db):
    ...
    result = runner.invoke(app, ["inquisitor", "--verbose", "--fix"])
    assert result.exit_code == 0
...
def test_inquisitor_detects_orphaned_task_possession_ref(initialized_project: Path):
    ...
    _raw_execute(
        db_path,
        """
        INSERT INTO tasks (id, title, status, priority, role, current_possession_id, claimed_at, file_path, created_at, updated_at)
...
def test_inquisitor_fixes_orphaned_task_possession_ref(initialized_project: Path):
    ...
    rows = _raw_query(db_path, "SELECT current_possession_id FROM tasks WHERE id = 'ENG-H-0002'")
    assert rows[0]["current_possession_id"] is None
...
def test_inquisitor_detects_underway_without_claimed_at(initialized_project: Path):
    ...
    _raw_execute(
        db_path,
        """
        INSERT INTO tasks (id, title, status, priority, role, claimed_at, current_possession_id, file_path, created_at, updated_at)
...
def test_inquisitor_detects_abandoned_task(initialized_project: Path):
    ...
    _raw_execute(
        db_path,
        """
        INSERT INTO possessions (id, daemon_name, role, possession_log, status, start_time, end_time, created_at, updated_at)
        VALUES (910, 'Abaddon', 'Engineer', '.opencode/work/possessions/ended.md', 'EXORCISED', datetime('now'), datetime('now'), datetime('now'), datetime('now'))
    """,
    )
...
def test_inquisitor_fixes_abandoned_task(initialized_project: Path):
    ...
    _raw_execute(
        ...
        """
        INSERT INTO possessions (id, daemon_name, role, possession_log, status, start_time, end_time, created_at, updated_at)
        VALUES (911, 'Abaddon', 'Engineer', '.opencode/work/possessions/ended2.md', 'EXORCISED', datetime('now'), datetime('now'), datetime('now'), datetime('now'))
    """,
    )
    ...
    rows = _raw_query(db_path, "SELECT current_possession_id FROM tasks WHERE id = 'ENG-H-0008'")
    assert rows[0]["current_possession_id"] is None
